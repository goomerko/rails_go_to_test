...
import RailsGoToTest.resolver
import re

class RailsGoToTestCommand(sublime_plugin.WindowCommand):

	def run(self):
		sublime.status_message('Running Rails Go To Test')
		win = self.window
		...
		related_files = RailsGoToTest.resolver.Resolver().run(current_file)
